...
preventiveMaintenanceController.showList({'show-load':true});
function pmValueToField(div){
    ...
    if(field == "text" || field == "date"){
        ...
        value="${current_val}" onblur="pmFieldToValue(this)" 
        ...
        pm-info-field" 
        onblur="pmFieldToValue(this)" id="${main.data('id')}" 
        data-id="${main.data('id')}"
...
function pmFieldToValue(div){
    ...
    const prev = `<a href="#" onclick="pmValueToField(this)" 
    ...
    class="pm-info-link">${present_value == ""?"N/A":present_value}</a>`;
